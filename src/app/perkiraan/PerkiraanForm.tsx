...
export default function PerkiraanForm({ isOpen, onClose, estimations, initialData, inline = false }: PerkiraanFormProps) {
  const formRef = useRef<HTMLFormElement>(null);
  
  const [selectedL1, setSelectedL1] = useState('');
  const [selectedL2, setSelectedL2] = useState('');
  const [selectedL3, setSelectedL3] = useState('');
  const [debetDisplay, setDebetDisplay] = useState('');
  const [debetValue, setDebetValue] = useState(0);
  const [kreditDisplay, setKreditDisplay] = useState('');
  const [kreditValue, setKreditValue] = useState(0);
  function onCurrencyChange(v: string, which: 'debet' | 'kredit') {
    const digits = v.replace(/\D/g, '');
    const num = digits ? Number(digits) : 0;
    if (which === 'debet') {
      setDebetValue(num);
      setDebetDisplay(digits ? `Rp. ${digits.replace(/\B(?=(\d{3})+(?!\d))/g, ',')}` : '');
    } else {
      setKreditValue(num);
      setKreditDisplay(digits ? `Rp. ${digits.replace(/\B(?=(\d{3})+(?!\d))/g, ',')}` : '');
    }
  }
  ...
  useEffect(() => {
    if (initialData && isOpen) {
        ...
        setSelectedL2(ref2);
        setSelectedL3(ref3);
        const d = initialData.debet || 0;
        const k = initialData.kredit || 0;
        setDebetValue(d);
        setKreditValue(k);
        setDebetDisplay(d ? `Rp. ${Math.floor(d).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}` : '');
        setKreditDisplay(k ? `Rp. ${Math.floor(k).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}` : '');
      ...
      {inline ? (
        <div className="bg-white p-6 rounded-lg shadow-sm">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-bold">{initialData ? 'Edit Perkiraan' : 'Input Perkiraan'}</h2>
          </div>
          <form ref={formRef} action={clientAction} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input 
                  ...
                  required 
                  className="mt-1 block w-full rounded-md border border-gray-300 p-2 shadow-sm focus:border-green-500 focus:ring-green-500 text-sm" 
              />
            </div>
               ...
               <div className="grid grid-cols-2 gap-4">
                   <div>
                      ...
                      <input
                          ...
                          placeholder="Rp. 0"
                          value={debetDisplay}
                          onChange={(e) => onCurrencyChange(e.target.value, 'debet')}
                          className="mt-1 block w-full rounded-md border border-gray-300 p-2 shadow-sm focus:border-green-500 focus:ring-green-500 text-sm"
                      />
                      <input type="hidden" name="debet" value={debetValue} />
                   </div>
                   <div>
                      ...
                      <input
                          ...
                          placeholder="Rp. 0"
                          value={kreditDisplay}
                          onChange={(e) => onCurrencyChange(e.target.value, 'kredit')}
                          className="mt-1 block w-full rounded-md border border-gray-300 p-2 shadow-sm focus:border-green-500 focus:ring-green-500 text-sm"
                      />
                      <input type="hidden" name="kredit" value={kreditValue} />
                   </div>
               </div>
            </div>
