...
export async function getAccounts() {
  await dbConnect();
  // Ambil hanya akun level 4 (akun transaksi) dari data Perkiraan
...
export async function getTransactionSettings() {
  await dbConnect();
  const settings = await TransactionSetting.find({})
    .populate('debit_account_id')
    .populate('credit_account_id');
  return JSON.parse(JSON.stringify(settings));
}

export async function saveTransactionSetting(formData: FormData) {
  await dbConnect();
  ...
  try {
    await TransactionSetting.findOneAndUpdate(
      { type },
      { 
        type,
